...
def plot_wfront(mwf, title_fig, isHlog, isVlog, i_x_min, i_y_min, orient, onePlot, bPlotPha=None,saveDir=None):
    ...
    #[x1, x2, y1, y2] = [-200, 200, 350, 550]
    print '[x1, x2, y1, y2]:', mwf.get_limits()
    pylab.imshow(ii, extent=[x1 * 1e3, x2 * 1e3, y1 * 1e3, y2 * 1e3])
    #pylab.imshow(ii, extent=[x1 * 1e3/3, x2 * 1e3/3, y1 * 1e3/3, y2 * 1e3/3])
    pylab.set_cmap('bone')
    #pylab.set_cmap('hot')
    pylab.axis('tight')
    ...
    if bPlotPha:
        ...
        #pylab.xlim(numpy.min(-30),numpy.max(30))  #[AKM]
       # pylab.ylim(-numpy.pi-1, numpy.pi+1) #[AKM]
        pylab.xlabel('y (mm)')
